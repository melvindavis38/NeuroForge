...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.boltVersion) {
      body += `- NeuroForge: ${data.environmentInfo.boltVersion}\n`;
    }
  ...
  body += '---\n*Submitted via NeuroForge bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'stackblitz-labs/NeuroForge';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'NeuroForge-bug-reporter',
    });
